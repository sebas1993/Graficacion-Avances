...
function move(e)
{
	...
	if (turnship.length == 0) {
		...
		if (power > maxspeedsq) {
			$('#board').css('cursor', 'not-allowed');
			$('#shotinfo').html('FuerA De Rango');
		} else {
			$('#board').css('cursor', 'crosshair');
			var pos = turnship.position();
			$('#shotinfo').html('Fuerza: ' + (Math.sqrt(power / maxspeedsq) * 100).toFixed(2) +
				'%<br />Angulo: ' + (Math.atan2(pos.left - e.pageX + margin,
...
function newship(x, y) {
	...
	if (x <= 0.5)
		ship.css('background-image', "url('ewj.gif')");
	else
		ship.css('background-image', "url('ewj1.gif')");
}
